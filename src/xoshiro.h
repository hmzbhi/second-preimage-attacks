...
/* basic wrappers for convenience, PK, 2018, 2023 */
...
static bool __my_little_xoshiro256starstar_init_was_done = false;
...
void __my_little_xoshiro256starstar_initialization(uint64_t iv[4])
{
    ...
    __my_little_xoshiro256starstar_init_was_done = true;
...
uint64_t __my_little_xoshiro256starstar__next__unsafe(void) {
	const uint64_t result_starstar = __my_little_xoshiro_rotl(__my_little_xoshiro256starstar__s[1] * 5, 7) * 9;
	...
	__my_little_xoshiro256starstar__s[3] = __my_little_xoshiro_rotl(__my_little_xoshiro256starstar__s[3], 45);

	return result_starstar;
}

uint64_t __my_little_xoshiro256starstar__next(void) {
	if (!__my_little_xoshiro256starstar_init_was_done)
	{
...
static bool __my_little_xoshiro256plus_init_was_done = false;
static uint64_t __my_little_xoshiro256plus__s[4];

/* Inits */

void __my_little_xoshiro256plus_initialization(uint64_t iv[4])
{
	__my_little_xoshiro256plus__s[0] = iv[0];
	__my_little_xoshiro256plus__s[1] = iv[1];
	__my_little_xoshiro256plus__s[2] = iv[2];
	__my_little_xoshiro256plus__s[3] = iv[3];
    __my_little_xoshiro256plus_init_was_done = true;

    return;
}

/* This function initializes one state with a key from /dev/urandom */
void __my_little_xoshiro256plus_unseeded_init()
{
	FILE *urd = fopen("/dev/urandom", "r");
	uint64_t iv[4] = {1,1,1,1};
	...
	{
		fprintf(stderr, "failed to initialize the little xoshiro256+ prng [No file called /dev/urandom]\n");
		__my_little_xoshiro256plus_initialization(iv);
		return;
	}
...
uint64_t __my_little_xoshiro256plus__next__unsafe(void) {
	const uint64_t result = __my_little_xoshiro256plus__s[0] + __my_little_xoshiro256plus__s[3];

	const uint64_t t = __my_little_xoshiro256plus__s[1] << 17;

	__my_little_xoshiro256plus__s[2] ^= __my_little_xoshiro256plus__s[0];
	__my_little_xoshiro256plus__s[3] ^= __my_little_xoshiro256plus__s[1];
	__my_little_xoshiro256plus__s[1] ^= __my_little_xoshiro256plus__s[2];
	__my_little_xoshiro256plus__s[0] ^= __my_little_xoshiro256plus__s[3];

	__my_little_xoshiro256plus__s[2] ^= t;

	__my_little_xoshiro256plus__s[3] = __my_little_xoshiro_rotl(__my_little_xoshiro256plus__s[3], 45);

	return result;
}

uint64_t __my_little_xoshiro256plus__next(void) {
	if (!__my_little_xoshiro256plus_init_was_done)
	{
		__my_little_xoshiro256plus_unseeded_init();
		__my_little_xoshiro256plus_init_was_done = true;
	}

	return __my_little_xoshiro256plus__next__unsafe();
}
	...
	for(uint64_t i = 0; i < sizeof JUMP / sizeof *JUMP; i++)
		for(int b = 0; b < 64; b++) {
			if (JUMP[i] & UINT64_C(1) << b) {
				...
				s1 ^= __my_little_xoshiro256plus__s[1];
				s2 ^= __my_little_xoshiro256plus__s[2];
				s3 ^= __my_little_xoshiro256plus__s[3];
			}
			__my_little_xoshiro256plus__next();	
		}
		
	__my_little_xoshiro256plus__s[0] = s0;
	__my_little_xoshiro256plus__s[1] = s1;
	__my_little_xoshiro256plus__s[2] = s2;
	__my_little_xoshiro256plus__s[3] = s3;
}
...
{
	return __my_little_xoshiro256plus__next();
}
...
{
	__my_little_xoshiro256plus_initialization(seed);
}
#endif
